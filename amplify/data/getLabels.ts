...
export const handler: Schema["getLabels"]["functionHandler"] = async (
  ...
  let filteredLabels = response.Labels?.filter((label) => label.Instances && label.Instances.length > 0);

  // remove every label that has a confidence score less than 50
  filteredLabels = filteredLabels?.filter((label) => label.Confidence && label.Confidence >= 50);
  ...
  filteredLabels = filteredLabels?.map((label) => {
    ...
    const highestConfidenceInstance = label.Instances.reduce((max, instance) => (max.Confidence && instance.Confidence && max.Confidence > instance.Confidence ? max : instance), label.Instances[0]);
    return {
      ...label,
  ...
  const result = filteredLabels?.map((label) => {
    if (!label.Instances ||
